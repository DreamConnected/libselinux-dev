...
static int seapp_context_cmp(const void *A, const void *B)
{
	...
	dup = (!s1->user.str || !strcmp(s1->user.str, s2->user.str)) &&
		...
		(s1->separateUidOnly && s1->separateUidOnly == s2->separateUidOnly);
...
int seapp_context_reload_internal(const path_alts_t *context_paths)
{
		...
		while (fgets(line_buf, sizeof line_buf - 1, fp)) {
			...
			while (1) {
				...
				*value++ = 0;
				...
				} else if (!strcasecmp(name, "separateUidOnly")) {
					if (!strcasecmp(value, "true"))
						cur->separateUidOnly = true;
					else if (!strcasecmp(value, "false"))
						cur->separateUidOnly = false;
					else {
						free_seapp_context(cur);
						goto err;
					}
...
#if DEBUG
	{
		...
		for (i = 0; i < nspec; i++) {
			...
			selinux_log(SELINUX_INFO, "%s:  isSystemServer=%s isEphemeralApp=%s separateUidOnly=%s user=%s seinfo=%s "
				...
				cur->separateUidOnly ? "true" : "false",
				cur->domain, cur->type, cur->level,
				cur->levelFromName[cur->levelFrom]);
		}
	}
#endif
...
#define SEPARATE_UID_ONLY_STR ":separateUidOnly"
				...
				context_t ctx)
{
	...
	if (seinfo) {
		...
		separateUidOnly = strstr(seinfo, SEPARATE_UID_ONLY_STR) ? true : false;
	...
	for (i = 0; i < nspec; i++) {
		...
		if (cur->separateUidOnly != separateUidOnly)
			continue;
