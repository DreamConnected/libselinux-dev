...
static const struct selinux_opt seopts_file_mainline[] = {
    { SELABEL_OPT_PATH, "/dev/selinux/mainline_file_contexts" },
    { SELABEL_OPT_PATH, "/mainline_file_contexts" }
};
...
struct selabel_handle* selinux_android_file_context_handle(void)
{
    ...
    for (i = 0; i < ARRAY_SIZE(seopts_file_mainline); i++) {
        if (access(seopts_file_mainline[i].value, R_OK) != -1) {
            seopts_file[size++] = seopts_file_mainline[i];
            break;
        }
    }
