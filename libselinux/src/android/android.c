...
struct selabel_handle* selinux_android_keystore_key_open_context_handle(const struct selinux_opt* seopts_service,
                                                                   unsigned nopts)
{
    struct selabel_handle* sehandle;

    sehandle = selabel_open(SELABEL_CTX_ANDROID_KEYSTORE_KEY,
            seopts_service, nopts);
    ...
    selinux_log(SELINUX_INFO, "SELinux: Loaded keystore_key_contexts from:\n");
    for (unsigned i = 0; i < nopts; i++) {
        selinux_log(SELINUX_INFO, "    %s\n", seopts_service[i].value);
    }
    return sehandle;
...
struct selabel_handle* selinux_android_keystore_key_context_handle(void)
{
    ...
    return selinux_android_keystore_key_open_context_handle(seopts_service, size);
}
