...
/* Paths that are credential encrypted. restorecon should not be run on these
 * during early boot as the content may not have been decrypted yet.
 */
 ...
 */
int extract_userid(const char **pathname, unsigned int *userid)
{
	char *end = NULL;
