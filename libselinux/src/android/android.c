...
static bool compute_file_contexts_hash(uint8_t c_digest[], const struct selinux_opt *opts, unsigned nopts)
{
    ...
    for (i = 0; i < nopts; i++) {
        fd = open(opts[i].value, O_CLOEXEC | O_RDONLY | O_NOFOLLOW);
        if (fd < 0) {
...
struct selabel_handle* selinux_android_file_context(const struct selinux_opt *opts,
							unsigned nopts)
{
...
{
    if (selinux_android_opts_file_exists(seopts_system_file)) {
        return selinux_android_file_context(seopts_system_file,
					ARRAY_SIZE(seopts_system_file));
    } else {
        return selinux_android_file_context(seopts_rootfs_file,
					ARRAY_SIZE(seopts_rootfs_file));
    }
}
