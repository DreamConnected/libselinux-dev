...
static const struct selinux_opt seopts_service_odm[] = {
    { SELABEL_OPT_PATH, "/odm/etc/selinux/odm_service_contexts" },
    { SELABEL_OPT_PATH, "/odm_service_contexts" }
};
...
struct selabel_handle* selinux_android_service_context_handle(void)
{
    ...
    for (i = 0; i < ARRAY_SIZE(seopts_service_odm); i++) {
        if (access(seopts_service_odm[i].value, R_OK) != -1) {
            seopts_service[size++] = seopts_service_odm[i];
            break;
        }
    }
#endif
