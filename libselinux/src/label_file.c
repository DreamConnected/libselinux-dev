				      ...
				      size_t *match_count)
{
	...
	for (i = data->nspec - 1; i >= 0; i--) {
		 ...
		 * a regex check        */
		bool ignore_stem = match_count != NULL || partial;
		bool stem_matches = spec->stem_id == -1 || spec->stem_id == file_stem;
		// Don't check the stem if we want to find all matches or a partial
		// match. Otherwise the case "/data/misc/(/.*)?" will be considered
		// a miss for "/data".
