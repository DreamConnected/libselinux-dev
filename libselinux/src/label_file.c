		...
		unsigned n)
{
	...
	int status = -1;
	size_t i;
	bool baseonly = false, path_provided;
	...
	path = malloc(sizeof(*path) * num_paths);
	if (path == NULL) {
		goto finish;
	}
	memset(path, 0, sizeof(*path) * num_paths);
	rec->spec_files = path;
	...
	if (!baseonly) {
		status = process_file(path[0], "homedirs", rec, prefix,
							    rec->digest);
		if (status && errno != ENOENT)
			goto finish;

		status = process_file(path[0], "local", rec, prefix,
							    rec->digest);
		if (status && errno != ENOENT)
			goto finish;
	}
