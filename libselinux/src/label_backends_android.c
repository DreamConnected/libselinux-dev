...
static struct selabel_lookup_rec *keystore_key_lookup(struct selabel_handle *rec,
		const char *key, int __attribute__((unused)) type)
{
	struct saved_data *data = (struct saved_data *)rec->data;
	spec_t *spec_arr = data->spec_arr;
...
int selabel_keystore_key_init(struct selabel_handle *rec,
		const struct selinux_opt *opts, unsigned nopts)
{
	struct saved_data *data;
	...
	rec->func_close = &closef;
	rec->func_stats = &stats;
	rec->func_lookup = &keystore_key_lookup;
