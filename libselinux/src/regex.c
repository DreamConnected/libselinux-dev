...
void regex_data_free(struct regex_data *regex)
{
	if (regex) {
		...
		pthread_mutex_destroy(&regex->match_mutex);
		free(regex);
	}
}
...
struct regex_data *regex_data_create(void)
{
	...
	pthread_mutex_init(&regex_data->match_mutex, NULL);
	return regex_data;
}
