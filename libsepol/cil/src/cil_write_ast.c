...
int __cil_unfill_expr_helper(struct cil_list_item *curr,
			     struct cil_list_item **next, char **out_str) {
	int rc = SEPOL_ERR;
	char *str = NULL;
	char *operand1 = NULL;
	char *operand2 = NULL;
	...
	switch(curr->flavor) {
	case CIL_LIST:
		rc = cil_unfill_expr((struct cil_list *)curr->data, &str, 1);
		if (rc != SEPOL_OK)
			goto exit;
		*out_str = str;
		*next = curr->next;
	...
	case CIL_DATUM:
		...
		cil_log(CIL_INFO, "Encountered CIL_DATUM in str_expr unfilling\n");
	...
	case CIL_OP: {
		...
		if (op_flavor == CIL_ALL) {
			...
			*next = curr->next;
			...
			rc = __cil_unfill_expr_helper(curr->next, next, &operand1);
			if (rc != SEPOL_OK)
				goto exit;
			...
			*out_str = cil_malloc(len);
			...
			rc = __cil_unfill_expr_helper(*next, next, &operand2);
			if (rc != SEPOL_OK)
				goto exit;
...
int cil_unfill_expr(struct cil_list *expr_str, char **out_str, int paren) {
	...
	while(curr != NULL) {
		...
		rc = __cil_unfill_expr_helper(curr, &next, &str);
        if (rc != SEPOL_OK)
            goto exit;
...
int cil_unfill_cats(struct cil_cats *cats, char **out_str) {
	/* TODO look into why this gets an extra paren without '0' arg */
...
int cil_write_avrule(struct cil_tree_node *node, FILE *cil_out) {
	...
	/* TODO: add xperm support */
	if (avrule->is_extended) {
		cil_log(CIL_ERR, "avrulexperm support not yet implemented.\n");
		rc = SEPOL_ERR;
		goto exit;
	}
...
int __cil_write_node_helper(struct cil_tree_node *node, uint32_t *finished, void *extra_args)
{
	...
	case CIL_TYPEATTRIBUTESET:
		...
		break;
	case CIL_TYPEALIAS:
		fprintf(cil_out, "CIL_TYPEALIAS ");
		break;
	case CIL_TYPEALIASACTUAL:
		fprintf(cil_out, "CIL_TYPEALIASACTUAL ");
		break;
	case CIL_ROLETRANSITION:
		fprintf(cil_out, "CIL_ROLETRANSITION ");
		break;
	case CIL_NAMETYPETRANSITION:
		fprintf(cil_out, "CIL_NAMETYPETRANSITION ");
		break;
	...
	case CIL_AVRULE:
		...
		break;
	case CIL_AVRULEX:
		fprintf(cil_out, "CIL_AVRULEX ");
		break;
	case CIL_PERMISSIONX:
		fprintf(cil_out, "CIL_PERMISSIONX ");
		break;
	case CIL_TYPE_RULE:
		fprintf(cil_out, "CIL_TYPE_RULE ");
		break;
	case CIL_SENS:
		rc = cil_write_sens(node, cil_out);
		break;
	case CIL_SENSALIAS:
		fprintf(cil_out, "CIL_SENSALIAS ");
		break;
	case CIL_SENSALIASACTUAL:
		fprintf(cil_out, "CIL_SENSALIASACTUAL ");
		break;
	case CIL_CAT:
		rc = cil_write_cat(node, cil_out);
		break;
	case CIL_CATALIAS:
		fprintf(cil_out, "CIL_CATALIAS ");
		break;
	case CIL_CATALIASACTUAL:
		fprintf(cil_out, "CIL_CATALIASACTUAL ");
		break;
	case CIL_CATSET:
