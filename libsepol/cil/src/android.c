...
int binary_amend_helper(struct cil_tree_node *node,
                        uint32_t *finished __attribute__((unused)),
                        void *extra_args) {
  int rc = SEPOL_OK;
  ...
  case 1:
    switch (node->flavor) {
    case CIL_TYPE:
	  struct cil_type *data = node->data;
...
int cil_amend_policydb(cil_db_t *db, sepol_policydb_t *policydb) {
  int rc = SEPOL_ERR;
