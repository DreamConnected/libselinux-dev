...
int cil_android_compile_policy(void **data, size_t *pol_len,
                               const char *pol_files[], size_t num_files) {
	int rc = SEPOL_ERR;
	...
	for (int i = 0; i < num_files; i++) {
		infile = fopen(pol_files[i], "r");
		if (!infile) {
			cil_log(CIL_ERR, "Could not open file: %s\n", pol_files[i]);
			rc = SEPOL_ERR;
			goto exit;
		}
		rc = stat(pol_files[i], &sb);
		if (rc == -1) {
	...
	*data = cil_malloc(*pol_len);
	sepol_policy_file_set_mem(pf,(char *) *data, *pol_len);
	rc = sepol_policydb_write(pdb, pf);
	if (rc != 0) {
		cil_log(CIL_ERR, "Failed to write binary policy: %d\n", rc);
		goto exit;
