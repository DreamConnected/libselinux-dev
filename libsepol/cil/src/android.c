...
static int binary_amend_helper(struct cil_tree_node *node, uint32_t *finished __attribute__((unused)), void *extra_args)
{
	...
	case 1:
		...
		case CIL_TYPE:
			type = node->data;
			rc = cil_type_to_policydb(pdb, type, type_value_to_cil);
		...
		case CIL_TYPEATTRIBUTE:
			...
			typeattribute->used = CIL_TRUE;
			typeattribute->keep = CIL_TRUE;
			rc = cil_typeattribute_to_policydb(pdb, node->data, type_value_to_cil);
			// symtab_insert returns 1 when the symbol already exists
			if (rc == 1) {
				cil_log(CIL_ERR, "Typeattribute with symbol \"%s\" already exists.\n", typeattribute->datum.fqn);
				rc = SEPOL_OK;
			}
			break;
		case CIL_ROLE:
