...
int avrule_sort_xperms(struct av_xperm_range_list **rangehead)
{
...
int avrule_merge_xperms(struct av_xperm_range_list **rangehead)
{
...
int avrule_read_xperm_ranges(struct av_xperm_range_list **rangehead)
{
	char *id;
...
int avrule_omit_xperms(struct av_xperm_range_list **rangehead)
{
...
int avrule_xperm_ranges(struct av_xperm_range_list **rangelist)
{
...
int avrule_ioctl_partialdriver(struct av_xperm_range_list *rangelist,
...
int avrule_ioctl_completedriver(struct av_xperm_range_list *rangelist,
			av_extended_perms_t **extended_perms)
{
	struct av_xperm_range_list *r;
	av_extended_perms_t *xperms;
...
int avrule_xperms_single_driver(struct av_xperm_range_list *rangelist,
...
{
	avrule_t *avrule;
	struct av_xperm_range_list *rangelist;
...
int define_te_avtab_netlink(avrule_t *avrule_template)
{
	...
	// Netlink message types comfortably fit into a single driver (see RTM_MAX
	// in uapi/linux/rtnetlink.h)
...
int define_te_avtab_extended_perms(int which)
{
	...
	if (strcmp(id,"ioctl") == 0) {
		...
		if (define_te_avtab_ioctl(avrule_template))
			return -1;
	} else if (strcmp(id, "nlmsg_read") == 0
		|| strcmp(id, "nlmsg_readpriv") == 0
		|| strcmp(id, "nlmsg_write") == 0) {
		free(id);
		if (define_te_avtab_netlink(avrule_template))
			return -1;
	} else {
